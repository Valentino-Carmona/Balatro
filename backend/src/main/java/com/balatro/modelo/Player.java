...
public class Player implements JokerObserver {
    ...
    public Score playHand(Round round) {
        try {
            ...
            for (JokerApply joker : this.jokers) {
                int oldPoints = score.getPoints();
                float oldMult = score.getMult();
                joker.apply(score);
                if (score.getPoints() != oldPoints || score.getMult() != oldMult) {
                    score.logEvent("joker", joker.getNombre(), score.getPoints() - oldPoints, score.getMult() - oldMult);
                }
    ...
    public void reorderJoker(String jokerName, int direction) {
        int index = -1;
        for (int i = 0; i < jokers.size(); i++) {
            if (jokers.get(i).getNombre().equals(jokerName)) {
                index = i;
                break;
            }
        }
        if (index != -1) {
            ...
            if (newIndex >= 0 && newIndex < jokers.size()) {
                JokerApply temp = jokers.get(index);
                jokers.set(index, jokers.get(newIndex));
                jokers.set(newIndex, temp);
            }
        }
    }
